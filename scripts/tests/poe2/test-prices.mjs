...
import { enrichPriceMetadata, exchangeToPrices, mergePrices, normalizeItemClass, scoutToPrices, selectTrackedLeagues, slugifyLeague, stashToPrices } from "../../poe2/prices.mjs";
...
const enriched = enrichPriceMetadata({ NewBase: { exalted: 1, source: "PoE2Scout" } }, {
  "Metadata/Items/Test/NewBase": { name: "NewBase", item_class: "Helmet", tags: ["armour"] },
});
assert.equal(enriched.NewBase.itemClass, "Helmet", "RePoE metadata enriches non-GGG quotes by stable base name");
assert.deepEqual(enriched.NewBase.tags, ["armour"]);
