...
function shortConfidence(level) {
  return level === "high" ? "High" : level === "medium" ? "Caution" : level === "low" ? "Low" : "Unknown";
}
...
function routeEvidence(route, { minTurnoverExalted, minItemVolume, routeGap = 0 }) {
  ...
  const gapTone = routeGap > .5 ? "low" : routeGap > .2 ? "medium" : "high";
  return [
    { text: `${number(route.itemVolume, 0)} units/h`, tone: unitTone, title: "Completed item units in the market hour" },
    { text: `${number(route.limitingTurnoverExalted, 0)} ex/h limit`, tone: turnoverTone, title: "Turnover of the thinner route leg" },
    { text: `${unsignedPercent(route.rangePercent)} range`, tone: rangeTone, title: "Width of the completed value range" },
    ...(routeGap > .005 ? [{ text: `${unsignedPercent(routeGap)} route gap`, tone: gapTone, title: "Difference between the lowest and highest eligible route means" }] : []),
  ];
}
...
export default function CurrencyExchange({ initialItemId = "", league, priceData, exchange, history, currency, chaosExalted, rateSummary }) {
  ...
  const routeDepths = useMemo(() => new Map((selected?.routeOptions || []).map((route) => [route.quoteId, assessRouteDepth(route, selected.routeOptions)])), [selected]);
  ...
  const bestRouteEvidence = routeEvidence(recommendedRoute, {
    minTurnoverExalted: routeMinimum,
    minItemVolume: routeMinimumUnits,
    routeGap: viableRouteGap,
  });
        ...
        <section className="p2ex-router" ref={workbenchRef}>
          ...
          {selected ? <>
            ...
            <div className="p2ex-route-layout">
              <div className="p2ex-route-table p2ex-table-wrap"><table><thead><tr><th>{tradeSide === "buy" ? "Pay with" : "Receive"}</th><th>Rate per item</th><th>Total for {number(plannedUnits, 0)}</th><th>Exalted equivalent</th><th>Completed range</th><th>Route depth</th></tr></thead><tbody>{shownRoutes.map((route) => { const recommended = route.quoteId === recommendedRoute?.quoteId; const core = CORE_ROUTE_IDS.includes(route.quoteId); const comparing = route.quoteId === comparisonRouteId; const depth = routeDepths.get(route.quoteId); const routeConfidence = assessExchangeRoute(route, { minTurnoverExalted: routeMinimum, minItemVolume: routeMinimumUnits }); return <tr key={route.quoteId} className={`${recommended ? "recommended" : ""} ${core ? "core" : ""} ${comparing ? "comparison" : ""}`}><td><button type="button" className="p2ex-route-pick" aria-pressed={comparing} title={`Compare ${route.quoteName} with the best observed route`} onClick={() => setComparisonRouteId((current) => current === route.quoteId ? "" : route.quoteId)}><span className="p2ex-route-name"><strong>{route.quoteName}</strong><em className={`p2ex-route-confidence ${depth.level}`} title={depth.reason}>Depth: {depth.label}</em><em className={`p2ex-route-confidence ${routeConfidence.level}`} title={routeConfidence.reasons.join(" · ")}>Confidence: {shortConfidence(routeConfidence.level)}</em>{core && <em className="p2ex-pinned">Pinned</em>}</span><small>{recommended ? core ? "Best observed · Always shown" : "Best observed" : core ? `${route.routeLabel} · Always shown` : route.routeLabel}</small></button></td><td>{number(route.rateQuotePerItem)} {route.quoteName}</td><td>{number(route.rateQuotePerItem * plannedUnits)} {route.quoteName}</td><td>{number(route.priceExalted)} ex/item</td><td>{number(route.lowExalted)}–{number(route.highExalted)} ex<small>{unsignedPercent(route.rangePercent)} spread</small></td><td>{number(route.itemVolume, 0)} units/h<small>{number(route.limitingTurnoverExalted, 0)} ex/h limiting</small></td></tr>; })}</tbody></table>{remainingRoutes > 0 && <button type="button" className="p2ex-route-more" onClick={() => setVisibleRouteCount((count) => count + ROUTE_PAGE_SIZE)}>View {Math.min(ROUTE_PAGE_SIZE, remainingRoutes)} more <span>· {remainingRoutes} remaining</span></button>}</div>
            </div>
            <p className="p2ex-router-note"><strong>Not a live arbitrage quote.</strong> Depth compares completed units and limiting turnover against all routes for this item and hour, independently of filters and pagination. High means both rank in the top quarter with minimum absolute flow. Confidence also checks the price range. Routes compare same-hour completed means, and their legs may have cleared at different moments. Both legs must pass the selected turnover and unit floors; wide ranges and extreme disagreement reduce confidence.</p>
...
const css = `
.p2ex-evidence-bar{display:flex;align-items:center;flex-wrap:wrap;gap:8px 16px;padding:12px 15px;border:1px solid #652d2b;border-radius:7px;background:#190e0d;color:#e2e2e0;font-size:12.5px}.p2ex-evidence-bar>div{display:flex;gap:7px;flex-wrap:wrap}.p2ex-evidence-bar button{padding:7px 10px;border:1px solid #803b39;border-radius:4px;background:#251010;color:#e2e2e0;cursor:pointer}.p2ex-evidence-bar small{flex-basis:100%;color:#e2e2e0;font-size:12.5px}.p2ex-history-evidence{margin:0;color:#e2e2e0;font-size:12.5px}.p2ex-ranges{flex-wrap:wrap}.p2ex-recommendation.evidence-low{border-color:#7b3634;background:linear-gradient(100deg,#241010,#100c09)}.p2ex-recommendation.evidence-low>div>strong{color:#e48d8a}.p2ex-recommendation.evidence-medium{border-color:#793735}
.p2ex-main{display:grid;gap:14px}.p2ex-head{display:flex;align-items:end;justify-content:space-between;gap:24px;padding:20px 22px;border:1px solid #3e201f;border-radius:8px;background:linear-gradient(105deg,#170e0d,#0d0908)}.p2ex-head>div:first-child>span,.p2ex-chart-head span,.p2ex-routes header span,.p2ex-filters span,.p2ex-move span{color:#e2e2e0;font-size:12.5px;font-weight:700;letter-spacing:.12em;text-transform:uppercase}.p2ex-head h2{margin:4px 0;color:#e2e2e0;font-size:27px}.p2ex-head p{max-width:690px;margin:0;color:#e2e2e0;font-size:13px}.p2ex-kpis{display:grid;grid-template-columns:repeat(2,minmax(110px,1fr));gap:8px}.p2ex-kpis div{display:grid;gap:3px;padding:9px 11px;border:1px solid #3e201f;border-radius:5px;background:#100b09}.p2ex-kpis span{color:#e2e2e0;font-size:12.5px;text-transform:uppercase}.p2ex-kpis strong{color:#e2e2e0;font-size:16px}.p2ex-chart-card,.p2ex-market-card,.p2ex-routes{display:grid;gap:12px;padding:18px;border:1px solid #452221;border-radius:8px;background:#110c0a}.p2ex-chart-head{display:flex;justify-content:space-between;gap:18px}.p2ex-chart-head h3,.p2ex-routes h3{margin:4px 0;color:#e2e2e0;font-size:20px}.p2ex-chart-head p,.p2ex-routes header p{margin:0;color:#e2e2e0;font-size:12.5px}.p2ex-move{display:grid;align-content:start;gap:4px;text-align:right}.p2ex-move strong{font-size:20px}.p2ex-tools{display:flex;align-items:center;justify-content:space-between;gap:12px}.p2ex-ranges{display:flex;gap:5px}.p2ex-ranges button{padding:6px 10px;border:1px solid #452120;border-radius:4px;background:#160c0b;color:#e2e2e0;cursor:pointer}.p2ex-ranges button.on{border-color:#a43431;background:#2a0f0e;color:#efa29f}.p2ex-tools label{color:#e2e2e0;font-size:12.5px;cursor:pointer}.p2ex-tools label.disabled{opacity:.45;cursor:not-allowed}.p2ex-tools input{accent-color:#a43431}.p2ex-chart{min-height:280px;border-top:1px solid #301a1a;border-bottom:1px solid #301a1a}.p2ex-chart-empty,.p2ex-empty{display:grid;min-height:240px;place-items:center;color:#e2e2e0;font-size:13px}.p2ex-filters{display:grid;grid-template-columns:minmax(180px,1fr) 190px 190px;gap:8px}.p2ex-filters label{display:grid;gap:5px}.p2ex-filters input,.p2ex-filters select{min-width:0;padding:8px;border:1px solid #512523;border-radius:5px;background:#110b09;color:#e2e2e0}.p2ex-table-wrap{overflow:auto;border:1px solid #301a1a;border-radius:5px}.p2ex-table-wrap table{width:100%;border-collapse:collapse;white-space:nowrap}.p2ex-table-wrap th{padding:8px 10px;background:#160d0c;color:#e2e2e0;font-size:12.5px;letter-spacing:.08em;text-align:right;text-transform:uppercase}.p2ex-table-wrap th:first-child,.p2ex-table-wrap td:first-child{text-align:left}.p2ex-table-wrap td{padding:9px 10px;border-top:1px solid #2b1818;color:#e2e2e0;font-size:12.5px;text-align:right}.p2ex-table-wrap tbody tr{cursor:pointer}.p2ex-table-wrap tbody tr:hover,.p2ex-table-wrap tbody tr.selected{background:#210f0f}.p2ex-table-wrap td strong{display:block;color:#e2e2e0}.p2ex-table-wrap td small{display:block;margin-top:2px;color:#e2e2e0;font-size:12.5px}.p2ex-routes header{display:flex;align-items:end;justify-content:space-between;gap:18px}.p2ex-routes header p{max-width:620px;text-align:right}.p2ex-no-rows{padding:24px;color:#e2e2e0;font-size:12.5px;text-align:center}.gain{color:#e2e2e0!important}.loss{color:#e2e2e0!important}@media(max-width:980px){.p2ex-head{align-items:stretch;flex-direction:column}.p2ex-kpis{grid-template-columns:repeat(4,1fr)}.p2ex-filters{grid-template-columns:1fr 1fr}.p2ex-filters label:first-child{grid-column:1/-1}}@media(max-width:680px){.p2ex-kpis{grid-template-columns:1fr 1fr}.p2ex-chart-head,.p2ex-routes header{align-items:stretch;flex-direction:column}.p2ex-move,.p2ex-routes header p{text-align:left}.p2ex-tools{align-items:flex-start;flex-direction:column}.p2ex-filters{grid-template-columns:1fr}.p2ex-filters label:first-child{grid-column:auto}}
.p2ex-overview{display:grid;gap:12px;padding:18px;border:1px solid #5b2422;border-radius:8px;background:radial-gradient(circle at 88% 0,#2c100f 0,transparent 30%),#100b09}.p2ex-overview-head{display:flex;align-items:end;justify-content:space-between;gap:22px}.p2ex-overview-head>div>span,.p2ex-signal-panel header>span{color:#e89e9b;font-size:12.5px;font-weight:700;letter-spacing:.13em;text-transform:uppercase}.p2ex-overview-head h2{margin:4px 0;color:#e2e2e0;font-size:23px}.p2ex-overview-head p{max-width:720px;margin:0;color:#e2e2e0;font-size:12.5px}.p2ex-overview-head dl{display:flex;gap:18px;margin:0}.p2ex-overview-head dl div{display:grid;gap:3px;text-align:right}.p2ex-overview-head dt{color:#e2e2e0;font-size:12.5px;text-transform:uppercase}.p2ex-overview-head dd{margin:0;color:#e2e2e0;font-size:15px;font-weight:700}.p2ex-signal-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.p2ex-signal-panel{min-width:0;overflow:hidden;border:1px solid #3c2120;border-radius:7px;background:#120d0b}.p2ex-signal-panel header{display:grid;gap:3px;padding:13px 14px;border-bottom:1px solid #301a1a}.p2ex-signal-panel h3{margin:0;color:#e2e2e0;font-size:16px}.p2ex-signal-panel header p{margin:0;color:#e2e2e0;font-size:12.5px;line-height:1.35}.p2ex-signal-list{display:grid}.p2ex-signal-list button{display:grid;grid-template-columns:20px minmax(0,1fr) auto;align-items:center;gap:8px;padding:9px 11px;border:0;border-bottom:1px solid #281717;background:transparent;color:inherit;text-align:left;cursor:pointer}.p2ex-signal-list button:last-child{border-bottom:0}.p2ex-signal-list button:hover{background:#210f0f}.p2ex-signal-list button>em{color:#e2e2e0;font-size:12.5px;font-style:normal}.p2ex-signal-list button>span{display:grid;min-width:0;gap:2px}.p2ex-signal-list strong{overflow:hidden;color:#e2e2e0;font-size:12.5px;text-overflow:ellipsis;white-space:nowrap}.p2ex-signal-list small{overflow:hidden;color:#e2e2e0;font-size:12.5px;text-overflow:ellipsis;white-space:nowrap}.p2ex-signal-list b{color:#e2e2e0;font-size:12.5px;white-space:nowrap}.p2ex-signal-empty{padding:20px 14px;color:#e2e2e0;font-size:12.5px;text-align:center}@media(max-width:900px){.p2ex-overview-head{align-items:stretch;flex-direction:column}.p2ex-overview-head dl{justify-content:space-between}.p2ex-overview-head dl div{text-align:left}}@media(max-width:680px){.p2ex-signal-grid{grid-template-columns:1fr}.p2ex-overview-head dl{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}}
.p2ex-workbench{display:grid;gap:14px;padding:18px;border:1px solid #652d2b;border-radius:8px;background:linear-gradient(120deg,#170d0c,#100b09)}.p2ex-workbench-head{display:flex;align-items:end;justify-content:space-between;gap:20px}.p2ex-workbench-head>div:first-child>span,.p2ex-planner-controls label>span,.p2ex-planner-controls legend,.p2ex-flow span,.p2ex-brief-grid span{color:#e2e2e0;font-size:12.5px;font-weight:700;letter-spacing:.1em;text-transform:uppercase}.p2ex-workbench h2{margin:4px 0;color:#e2e2e0;font-size:22px}.p2ex-workbench-head p{max-width:760px;margin:0;color:#e2e2e0;font-size:12.5px}.p2ex-flow{display:grid;flex:0 0 auto;gap:4px;padding:10px 12px;border:1px solid #492827;border-radius:6px;background:#100b09;text-align:right}.p2ex-flow strong{color:#e2e2e0;font-size:13px}.p2ex-flow.fits{border-color:#365a39}.p2ex-flow.fits strong{color:#e2e2e0}.p2ex-flow.large{border-color:#6a3332}.p2ex-flow.large strong{color:#e2e2e0}.p2ex-planner-controls{display:flex;align-items:end;gap:10px}.p2ex-planner-controls label{display:grid;gap:5px}.p2ex-planner-controls input{width:160px;padding:8px;border:1px solid #632c2b;border-radius:5px;background:#0f0a08;color:#e2e2e0}.p2ex-planner-controls fieldset{display:flex;gap:5px;margin:0;padding:0;border:0}.p2ex-planner-controls legend{margin-bottom:5px}.p2ex-planner-controls button{padding:8px 11px;border:1px solid #492827;border-radius:5px;background:#130d0a;color:#e2e2e0;cursor:pointer}.p2ex-planner-controls button.on{border-color:#a53935;background:#2a1110;color:#efa29f}.p2ex-brief-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.p2ex-brief-grid article{display:grid;min-width:0;gap:4px;padding:11px 12px;border:1px solid #382120;border-radius:6px;background:#100b09}.p2ex-brief-grid strong{overflow:hidden;color:#e2e2e0;font-size:17px;text-overflow:ellipsis;white-space:nowrap}.p2ex-brief-grid small{overflow:hidden;color:#e2e2e0;font-size:12.5px;text-overflow:ellipsis;white-space:nowrap}.p2ex-planner-note{margin:0;color:#e2e2e0;font-size:12.5px}@media(max-width:800px){.p2ex-workbench-head{align-items:stretch;flex-direction:column}.p2ex-flow{text-align:left}.p2ex-brief-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:520px){.p2ex-planner-controls{align-items:stretch;flex-direction:column}.p2ex-planner-controls input{box-sizing:border-box;width:100%}.p2ex-planner-controls fieldset{display:grid;grid-template-columns:repeat(4,1fr)}.p2ex-brief-grid{grid-template-columns:1fr}}
.p2ex-route-workspace{display:grid;grid-template-columns:minmax(340px,400px) minmax(0,1fr);align-items:start;gap:14px}.p2ex-router{display:grid;min-width:0;gap:14px;padding:20px;border:1px solid #6d2927;border-radius:8px;background:radial-gradient(circle at 90% 0,#2d1110 0,transparent 30%),#100b09}.p2ex-router-head{display:flex;align-items:end;justify-content:space-between;gap:20px}.p2ex-router-head>div:first-child>span,.p2ex-router-controls span,.p2ex-recommendation>div>span,.p2ex-route-leaders>span{color:#e89e9b;font-size:12.5px;font-weight:700;letter-spacing:.12em;text-transform:uppercase}.p2ex-router-head h2{margin:4px 0;color:#e2e2e0;font-size:25px}.p2ex-router-head p{max-width:760px;margin:0;color:#e2e2e0;font-size:12.5px}.p2ex-side-switch{display:flex;flex:0 0 auto;padding:3px;border:1px solid #512523;border-radius:6px;background:#0e0907}.p2ex-side-switch button{padding:8px 12px;border:0;border-radius:4px;background:transparent;color:#e2e2e0;cursor:pointer}.p2ex-side-switch button.on{background:#7b2421;color:#e2e2e0}.p2ex-router-controls{display:grid;grid-template-columns:repeat(3,minmax(130px,1fr));gap:8px}.p2ex-router-controls label{display:grid;align-content:start;gap:5px}.p2ex-router-controls label>small{color:#e2e2e0;font-size:12.5px}.p2ex-router-controls input,.p2ex-router-controls select{box-sizing:border-box;width:100%;min-width:0;padding:9px;border:1px solid #552725;border-radius:5px;background:#100a08;color:#e2e2e0}.p2ex-recommendation{position:relative;display:grid;grid-template-columns:minmax(260px,1fr) 1.3fr;gap:16px;padding:15px;border:1px solid #41603e;border-radius:7px;background:linear-gradient(100deg,#142017,#100c09)}.p2ex-recommendation>div{display:grid;align-content:center;gap:4px}.p2ex-recommendation>div>strong{color:#e2e2e0;font-size:22px}.p2ex-recommendation small{color:#e2e2e0;font-size:12.5px}.p2ex-recommendation dl{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin:0}.p2ex-recommendation dl div{display:grid;align-content:center;gap:4px;padding:9px 10px;border-left:1px solid #30402e}.p2ex-recommendation dt{color:#e2e2e0;font-size:12.5px;text-transform:uppercase}.p2ex-recommendation dd{margin:0;color:#e2e2e0;font-size:14px;font-weight:700}.p2ex-comparison{padding-right:122px;border-color:#793e3c;background:linear-gradient(100deg,#211010,#100c09)}.p2ex-comparison>div>strong{color:#e49a97}.p2ex-comparison-clear{position:absolute;top:10px;right:10px;padding:6px 8px;border:1px solid #5b2c2b;border-radius:4px;background:#160c0b;color:#e2e2e0;cursor:pointer;font-size:12.5px}.p2ex-comparison-clear:hover{border-color:#9b3d3a;color:#efa29f}.p2ex-route-layout{display:grid;grid-template-columns:minmax(0,1fr) 260px;gap:10px}.p2ex-route-table tr.recommended{background:#152016}.p2ex-route-table tr.recommended td:first-child strong{color:#e2e2e0}.p2ex-route-table tr.comparison{box-shadow:inset 3px 0 #c85855;background:#261010}.p2ex-route-pick{display:grid;width:100%;min-width:0;gap:2px;padding:0;border:0;background:transparent;color:inherit;cursor:pointer;text-align:left}.p2ex-route-pick:hover strong{color:#efa29f}.p2ex-route-more{display:block;width:100%;padding:11px;border:0;border-top:1px solid #3a2120;background:#160d0c;color:#e2e2e0;cursor:pointer;font-size:12.5px;font-weight:700;text-align:center}.p2ex-route-more:hover{background:#211010;color:#efa29f}.p2ex-route-more span{color:#e2e2e0;font-weight:400}.p2ex-route-leaders{display:grid;align-content:start;overflow:hidden;border:1px solid #3a2120;border-radius:6px;background:#110c0a}.p2ex-route-leaders>span,.p2ex-route-leaders>p{padding:0 12px}.p2ex-route-leaders>span{padding-top:12px}.p2ex-route-leaders>p{margin:4px 0 9px;color:#e2e2e0;font-size:12.5px}.p2ex-route-leaders>button{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:9px 12px;border:0;border-top:1px solid #2b1818;background:transparent;color:inherit;text-align:left;cursor:pointer}.p2ex-route-leaders>button:hover{background:#200f0f}.p2ex-route-leaders>button>span{display:grid;min-width:0;gap:2px}.p2ex-route-leaders strong{overflow:hidden;color:#e2e2e0;font-size:12.5px;text-overflow:ellipsis;white-space:nowrap}.p2ex-route-leaders small{color:#e2e2e0;font-size:12.5px}.p2ex-route-leaders b{color:#e79b98;font-size:12.5px}.p2ex-router-note{margin:0;color:#e2e2e0;font-size:12.5px}@media(max-width:980px){.p2ex-router-controls{grid-template-columns:1fr 1fr}.p2ex-route-layout{grid-template-columns:1fr}.p2ex-route-leaders{grid-template-columns:repeat(2,minmax(0,1fr))}.p2ex-route-leaders>span,.p2ex-route-leaders>p{grid-column:1/-1}}@media(max-width:780px){.p2ex-route-workspace{grid-template-columns:1fr}.p2ex-route-workspace>.p2mb-browser.sticky{position:static}}@media(max-width:720px){.p2ex-router-head{align-items:stretch;flex-direction:column}.p2ex-side-switch{align-self:flex-start}.p2ex-router-controls{grid-template-columns:1fr}.p2ex-recommendation{grid-template-columns:1fr}.p2ex-comparison{padding-right:15px;padding-top:48px}.p2ex-recommendation dl{grid-template-columns:1fr}.p2ex-recommendation dl div{border-top:1px solid #30402e;border-left:0}.p2ex-route-leaders{grid-template-columns:1fr}.p2ex-route-leaders>span,.p2ex-route-leaders>p{grid-column:auto}}
.p2ex-route-legend{display:flex;align-items:center;gap:8px;padding:8px 10px;border:1px solid #492726;border-radius:5px;background:#140e0b}
.p2ex-route-legend>span{flex:0 0 auto;padding:3px 6px;border:1px solid #9a3a37;border-radius:3px;background:#2b1110;color:#ef9693;font-size:12.5px;font-weight:700;letter-spacing:.08em;text-transform:uppercase}
.p2ex-route-legend p{margin:0;color:#e2e2e0;font-size:12.5px}
...
.p2ex-route-name em{flex:0 0 auto;padding:2px 5px;border:1px solid #8d3937;border-radius:3px;background:#2a1110;color:#eb9997;font-size:12.5px;font-style:normal;font-weight:700;letter-spacing:.07em;text-transform:uppercase}
.p2ex-route-table tr.core:not(.recommended):not(.comparison){box-shadow:inset 2px 0 #74312f}
...
.p2ex-market-picker{position:sticky;top:12px;min-width:0}.p2ex-market-picker-toggle{display:none}.p2ex-market-picker-body>.p2mb-browser{width:100%}.p2ex-router-controls{grid-template-columns:repeat(4,minmax(120px,1fr))}.p2ex-confidence-tags{display:flex;flex-wrap:wrap;gap:4px;margin-top:5px}.p2ex-confidence,.p2ex-confidence-tags span{display:block;width:max-content;max-width:100%;padding:4px 7px;border:1px solid #3d503a;border-radius:4px;background:#111a12;color:#e2e2e0;font-size:12.5px;font-style:normal;line-height:1.2}.p2ex-confidence.medium,.p2ex-confidence-tags span.medium{border-color:#6b312f;background:#1b0f0e;color:#d98987}.p2ex-confidence.low,.p2ex-confidence-tags span.low{border-color:#713331;background:#1d0f0e;color:#e2e2e0}.p2ex-route-confidence.high{border-color:#3d6841;background:#142017;color:#e2e2e0}.p2ex-route-confidence.medium{border-color:#762d2a;background:#210e0e;color:#db8684}.p2ex-route-confidence.low{border-color:#7b322f;background:#240f0f;color:#e2e2e0}.p2ex-router-note strong{color:#db8684}.p2ex-scanner-head{display:flex;align-items:end;justify-content:space-between;gap:18px}.p2ex-scanner-head>div>span{color:#e89e9b;font-size:12.5px;font-weight:700;letter-spacing:.12em;text-transform:uppercase}.p2ex-scanner-head h3{margin:4px 0;color:#e2e2e0;font-size:20px}.p2ex-scanner-head p{max-width:760px;margin:0;color:#e2e2e0;font-size:12.5px}.p2ex-scanner-head>strong{flex:0 0 auto;color:#e2e2e0;font-size:13px}.p2ex-filters{grid-template-columns:minmax(180px,1fr) repeat(3,minmax(150px,190px))}.p2ex-gap.high{color:#e2e2e0}.p2ex-gap.medium{color:#db8684}.p2ex-gap.low{color:#e2e2e0}.p2ex-gap small{font-weight:400}.p2ex-table-wrap th:last-child,.p2ex-table-wrap td:last-child{padding-right:14px}
...
@media(max-width:780px){.p2ex-market-picker{position:static}.p2ex-market-picker.collapsed{position:sticky;top:0;z-index:5}.p2ex-market-picker-toggle{display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:2px 12px;padding:11px 12px;border:1px solid #4a2321;border-radius:7px;background:rgba(17,12,10,.96);box-shadow:0 9px 24px #0008;backdrop-filter:blur(4px)}.p2ex-market-picker-toggle span{color:#e2e2e0;font-size:12.5px;letter-spacing:.1em;text-transform:uppercase}.p2ex-market-picker-toggle strong{overflow:hidden;color:#e2e2e0;font-size:13px;text-overflow:ellipsis;white-space:nowrap}.p2ex-market-picker-toggle button{grid-column:2;grid-row:1/3;padding:7px 9px;border:1px solid #632c2b;border-radius:4px;background:#210f0f;color:#e2e2e0;cursor:pointer}.p2ex-market-picker-body{margin-top:7px}.p2ex-market-picker.collapsed .p2ex-market-picker-body{display:none}.p2ex-router{scroll-margin-top:62px}}
