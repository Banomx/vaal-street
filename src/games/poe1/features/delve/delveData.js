/* ================================================================
   ...
   Three questions the Delve tab answers, and the data each needs:
     ...
     "what is a delve boss worth?" -> DELVE_BOSSES, in the same drop
                                      group shape ../bosses/bossData.js uses,
                                      so ../bosses/bossProfit.js prices them
...
export const BIOMES = [
  ...
  },
  {
    ...
    walls: ["Gilded Fossil", "Lucent Fossil"],
  ...
  },
  {
    ...
    walls: ["Sanctified Fossil", "Shuddering Fossil"],
...
/* ---------------- fossils ----------------
   ...
   Fractured walls are a property of the biome, not of the fossil: the wiki
   marks Fundamental as wall-locked in Magma Fissure but "Anywhere" in Sulphur
   Vents. Each biome therefore lists its own `walls`, and `wallIn` records where
   a fossil is locked away. `wall` marks a fossil that comes out of a wall
   anywhere at all, which is what the Fractured Walls pool is: what a wall can
   hand you. Fundamental qualifies on its Magma Fissure entry even though
   Sulphur Vents also drops it loose. */

function buildFossils() {
  const map = new Map();
  const touch = (name) => {
    if (!map.has(name)) map.set(name, { name, biomes: [], exclusive: null, wall: false, wallIn: [] });
    return map.get(name);
  };
  for (const b of BIOMES) {
    for (const f of b.pool) {
      const entry = touch(f);
      entry.biomes.push(b.id);
      if ((b.walls || []).includes(f)) entry.wallIn.push(b.id);
    }
    if (b.exclusive) {
  ...
  for (const f of map.values()) f.wall = f.wallIn.length > 0;
...
/* ---------------- nodes ----------------
   ...
   Stashes are priced from low/median/high outcomes in the
...
export const NODE_KINDS = {
  ...
  wall: { label: "Fractured Walls", tone: "#8f7fd4" },
...
export const NODES = [
  ...
  { id: "fractured-walls", name: "Fractured Walls", kind: "wall", biome: null,
    note: "Blown open in the darkness off any node. The only source of the wall-locked fossils." },
...
export const COMMUNITY_DEPTH_GUIDE = {
  ...
  /* A fractured wall is not a fossil node — it turns up in the darkness on the
     way to one — so this share is per node travelled to and does not compete
     with the special/stash split. Working figure, no depth ramp yet. */
...
export const GUIDE_SAMPLE = {
  ...
  stashQtyLow: 4,
  stashQtyHigh: 10,
  // Same for a fractured wall, on a narrower range.
  wallQtyLow: 4,
  wallQtyHigh: 6,
};
...
export const TUNABLES = [
  ...
  { key: "wallQtyLow", label: "Fewest fossils per fractured wall", group: "Per node", step: 1,
    source: "observed",
    help: "Four is the low end counted in current play. Walls are the only source of the wall-locked fossils." },
  { key: "wallQtyHigh", label: "Most fossils per fractured wall", group: "Per node", step: 1,
    source: "observed",
    help: "Six is the high end counted in current play. Preliminary — this range is expected to be refined." },
...
/* ---------------- delve bosses ----------------
   ...
   Same shape as ../bosses/bossData.js so ../bosses/bossProfit.js prices them. Each boss
  ...
  },
  {
    ...
    groups: [
      ...
      indep([
        ...
        // Preliminary Allflame estimate. Kurgal drops the Inevitable variant
        // only; the other three Eyes come from unrelated content and are not
        // part of this table.
        { key: "zorath", item: "Zorath's Eye of the Inevitable",
...
/* Declared prices for names poe.ninja doesn't carry, in the same shape
   ...
   by GGG first; poe.ninja and poe.watch cover the remaining item markets.
